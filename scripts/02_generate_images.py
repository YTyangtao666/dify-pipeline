...
async def run(products: list[dict], out_dir: Path, interval: float, limit_products: int) -> dict:
    cfg = Config.from_env()
...
def main():
    ...
    print(f"[02] 开始生图: {min(args.limit, len(products))} 商品 × {len(scraper.STYLES)} 风格 × {IMAGES_PER_PRODUCT} 张")

    manifest = asyncio.run(run(products, ROOT / args.out, args.interval, args.limit))
